...
# Updated May 2026: 1.5-flash removed (404), 2.0-flash retiring Jun 2026
MODELS_BY_PLAN = {
    "free":   ["gemini-2.5-flash",  "gemini-2.0-flash", "gemini-3.5-flash"],
    "pro":    ["gemini-2.5-pro",    "gemini-2.5-flash",  "gemini-3.5-flash", "gemini-2.0-flash"],
    "vertex": ["gemini-2.5-pro",    "gemini-2.5-flash",  "gemini-3.5-flash"],
}
...
def fetch_available_models(api_key: str) -> list[str]:
    """Fetch live model list from API and return generateContent-capable model IDs."""
    try:
        r = requests.get(LIST_MODELS_URL, params={"key": api_key}, timeout=10)
        ...
        # Prefer non-experimental, non-legacy ordering
        priority = ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-3.5-flash",
                    "gemini-2.0-flash", "gemini-3.5-flash-8b", "gemini-2.5-flash-8b"]
        ordered = [m for m in priority if m in capable]
        rest = [m for m in capable if m not in ordered
                and "exp" not in m and "preview" not in m and "001" not in m]
        return ordered + rest
    except Exception:
        return []
...
def _post_with_retry(url, payload, headers, params=None, max_retries=4, timeout=60):
    ...
    for attempt in range(max_retries):
        ...
        if r.status_code == 404:
            raise RuntimeError(f"MODEL_NOT_FOUND: {detail}")
...
def ask_gemini_prompt(quote, cfg):
    ...
    # Try to get live model list first; fall back to hardcoded list
    plan = cfg.get("plan", "free")
    if cfg["mode"] == "aistudio":
        live = fetch_available_models(cfg["key"])
    else:
        live = []
    fallback = MODELS_BY_PLAN.get(plan, MODELS_BY_PLAN["free"])
    # Merge: keep live-discovered models that match our preferred list first,
    # then append any remaining hardcoded ones not in live list
    models = live if live else fallback
    # Always include fallback models as safety net
    for m in fallback:
        if m not in models:
            models.append(m)
    last_err = None
    for model in models:
        try:
            ...
            if "MODEL_NOT_FOUND" in err_str or "404" in err_str:
                last_err = e; continue   # try next model silently
            raise
...
# Show available models once credentials are set
if cfg and cfg["mode"] == "aistudio":
    live_models = fetch_available_models(cfg["key"])
    if live_models:
        plan_label = cfg.get("plan", "free")
        preferred = MODELS_BY_PLAN.get(plan_label, [])
        top = [m for m in preferred if m in live_models]
        st.markdown(
            f'<p style="font-size:0.72rem;letter-spacing:0.1em;color:#5a5040;margin-bottom:1.5rem">'
            f'✓ API connected &nbsp;·&nbsp; {len(live_models)} models available &nbsp;·&nbsp; '
            f'Will use: <b style="color:#8a7060">{top[0] if top else live_models[0]}</b></p>',
            unsafe_allow_html=True
        )
